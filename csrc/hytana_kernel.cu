...
hytana_forward_cuda(
  ...
  torch::checkAllDefined("hytana_forward_cuda", {in_arg, out_arg});
  torch::checkAllSameGPU("hytana_forward_cuda", {in_arg, out_arg});
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.scalar_type(), "hytana_forward_cuda", [&] {
      kernel::hytana_forward<scalar_t>(output, input);
  });
}

void
hytana_backward_cuda(
  ...
  torch::checkAllDefined("hytana_backward_cuda", {gi_arg, in_arg, go_arg});
  torch::checkAllSameGPU("hytana_backward_cuda", {gi_arg, in_arg, go_arg});
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad_inp.scalar_type(), "hytana_backward_cuda", [&] {
      kernel::hytana_backward<scalar_t>(grad_inp, input, grad_out);
  });
}
