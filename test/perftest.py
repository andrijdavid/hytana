...
hytana_pt = lambda x, a=0.303, b=0.632: x.div(2).mul(1 + torch.tanh(a+(b*x)))
swish_pt = lambda x: x.div(1 + torch.exp(-x))
mish_pt = lambda x: x.mul(torch.tanh(torch.log1p(torch.exp(x))))
...
if __name__ == '__main__':
    ...
    for dtype in dtypes:
        ...
        timings = []
        funcs = {}
        if args.baseline: funcs.update(relu=torch.nn.functional.relu, softplus=torch.nn.functional.softplus, hytana_pt=hytana_pt, swish_pt=swish_pt, mish_pt=mish_pt)
        funcs['swish'] = SwishFunction.apply
        funcs['mish_cuda'] = MishCudaFunction.apply
        funcs['hytana'] = HytanaFunction.apply
        
        max_name = max(map(len, funcs.keys())) + 6
        print(ind+ind.ljust(max_name)+"Mean    ± Std     (Min  -  Max)")
