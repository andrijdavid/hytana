...
if "%DESIRED_PYTHON%" == "" set DESIRED_PYTHON=3.7
...
rmdir /s /q conda
del miniconda.exe
curl -k https://repo.continuum.io/miniconda/Miniconda3-latest-Windows-x86_64.exe --output "%miniconda_exe%"
...
for %%v in (%DESIRED_PYTHON_PREFIX%) do (
    ...
    set "CUDA_HOME=%ProgramFiles%\NVIDIA GPU Computing Toolkit\CUDA\v%CUDA_VERSION_STR%"
